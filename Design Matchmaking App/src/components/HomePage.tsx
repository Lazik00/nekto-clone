...
export function HomePage({
  ...
  // ----------------------------------------
  // POLL QUEUE AND NOTIFICATIONS
  // ----------------------------------------
  const pollQueueAndNotifications = async () => {
    try {
      // 1) Queue position
      const q = await fetch(getApiUrl("/api/v1/queue-status"), {
        headers: { Authorization: `Bearer ${accessToken}` },
      });

      if (q.ok) {
        const qData = await q.json();
        setQueuePosition(qData.position);
      }

      // 2) Notification check (match found)
      const n = await fetch(getApiUrl("/api/v1/notifications"), {
        headers: { Authorization: `Bearer ${accessToken}` },
      });

      if (n.ok) {
        const notifData = await n.json();
        const matchNotif = notifData.notifications?.find(
          (n: any) => n.type === "match_found"
        );
        ...
        if (matchNotif) {
          ...
          onMatchFound(matchNotif.session_id, matchNotif.match);
        }
      }
    } catch (err) {
  ...
  // ----------------------------------------
  // START SEARCH → SHOW PERMISSIONS FIRST
  // ----------------------------------------
  ...
  // ----------------------------------------
  // PERMISSIONS GRANTED → CALL /find ONE TIME
  // ----------------------------------------
  const handlePermissionsGranted = async () => {
    ...
    try {
      ...
      if (data.status === "queued") {
        // Start polling queue + notifications
        const interval = setInterval(pollQueueAndNotifications, 2000);
          ...
          <div className="max-w-2xl mx-auto px-4 py-20">
            ...
            <div className="grid md:grid-cols-3 gap-6 mt-16">
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 text-center border border-white/20">
                <Video className="w-6 h-6 text-white mx-auto mb-3" />
                <h3 className="text-white mb-2">Video Chat</h3>
                <p className="text-white/80 text-sm">
                  High-quality video chat worldwide
                </p>
              </div>

              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 text-center border border-white/20">
                <Search className="w-6 h-6 text-white mx-auto mb-3" />
                <h3 className="text-white mb-2">Smart Matching</h3>
                <p className="text-white/80 text-sm">
                  Matching based on intelligent preferences
                </p>
              </div>

              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 text-center border border-white/20">
                <UserCircle className="w-6 h-6 text-white mx-auto mb-3" />
                <h3 className="text-white mb-2">Safe & Secure</h3>
                <p className="text-white/80 text-sm">
                  Report and block features included
                </p>
              </div>
            </div>
          </div>
